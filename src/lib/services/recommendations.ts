...
export class RecommendationService {
  ...
  private async analyzeUserGenrePreferences(favorites: UserFavorite[]): Promise<GenreAnalysis> {
    ...
    for (const favorite of sampleFavorites) {
      try {
        const movieDetails: MovieDetails = await clientTMDB.getMovieDetailsSimple(favorite.movie_id);
        ...
        console.warn(`[RECOMMENDATIONS] Failed to get details for movie ${favorite.movie_id}:`, error);
      }
    }
  ...
  private async getGenreBasedRecommendations(
    ...
    for (const genreId of preferredGenres) {
      try {
        ...
        console.warn(`[RECOMMENDATIONS] Failed to get recommendations for genre ${genreId}:`, error);
      }
    }
  ...
  async getSimilarMovies(movieId: number, excludeIds: number[] = []): Promise<Movie[]> {
    try {
      ...
      return response.results
        .filter((movie: Movie) => !excludeSet.has(movie.id))
        .slice(0, 6);
    } catch (error) {
