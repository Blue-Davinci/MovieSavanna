...
function handleSignupError(error: AuthError): SignupResult {
 logError('SIGNUP_ERROR_HANDLER', {
  ...
  if (!errorInfo && error.message.includes('Email address') && error.message.includes('is invalid')) {
    errorInfo = { message: 'This email address is not allowed. Please try a different email address.' };
  }
...
function checkSignupRateLimit(email: string, clientIP: string): { allowed: boolean; attempts: number; waitTime: number } {
